...
class DimensionsCM(BaseModel):
    """Physical product dimensions measured in centimeters."""
...
class Seller(BaseModel):
    """Seller details required when creating a product."""
    id: UUID
    name: Annotated[
        str,
        Field(
    ...
    def validate_seller_email_domain(cls, value: EmailStr):
        """Allow seller emails only from approved business domains."""
        allowed_domains = {
            "mistore.in",
            "realmeofficial.in",
            "samsungindia.in",
...
class Product(BaseModel):
    """Validation model for creating and returning product records."""
    id: UUID
    sku: Annotated[
        str,
        Field(
    ...
    def validate_sku_format(cls, value: str):
        """Require an SKU with a hyphen and a three-digit suffix."""
        if "-" not in value:
    ...
    def validate_business_rules(cls, model: "Product"):
        """Enforce stock, activity, discount, and rating relationships."""
...
class SellerUpdate(BaseModel):
    ...
    @classmethod
    def validate_seller_email_domain(cls, value: EmailStr):
        """Keep update requests consistent with seller creation rules."""
        allowed_domains = {
            "mistore.in",
            "realmeofficial.in",
            "samsungindia.in",
...
class ProductUpdate(BaseModel):
    """Optional product fields accepted by the update endpoint."""
    ...
    @classmethod
    def validate_business_rules(cls, model: "Product"):
        """Validate business rules for fields included in an update request."""
        if model.stock == 0 and model.is_active is True:
            raise ValueError("If stock is 0, is_active must be false")
    ...
    @property
    def volume_cm3(self) -> float:
        """Calculate the updated dimensions in cubic centimeters."""
        d = self.dimensions_cm
        return round(d.length * d.width * d.height, 2)